...
import { registerUser, submitOtp, loginUser, googleLogin, facebookLogin, UserRegistrationCredentials, decodeAccessToken, logoutUser, fetchSession } from '../../src/authentication/authService'
...
describe('AuthService', () => {
  ...
  describe('submitOtp', () => {
    ...
    test('should return true if OTP confirmation is successful', async() => {
      ...
      expect(result).toEqual(expect.objectContaining({ isSucessRegister: true }))
    })
    ...
    test('should return false if OTP confirmation is not yet completed', async() => {
      ...
      const result = await submitOtp(email, otp)
      expect(result).toEqual(expect.objectContaining({ isSucessRegister: false }))
    })
  ...
  describe('loginUser', () => {
    ...
    test('should return true if login is successful', async() => {
      ...
      expect(result).toEqual(expect.objectContaining({ isSignedIn: true }))
    })
    ...
    test('should return false if login is not successful', async() => {
      ...
      const result = await loginUser(email, password)
      expect(result).toEqual(expect.objectContaining({ isSignedIn: false }))
    })
