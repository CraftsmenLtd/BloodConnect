...
const URGENCY_FACTOR_WEIGHT = 7

function calculateRemainingBagsNeeded(bloodQuantity: number, donorsFoundCount: number): number {
  return Math.max(0, bloodQuantity - donorsFoundCount)
}

function calculateTotalDonorsToNotify(remainingBagsNeeded: number, urgencyLevel: UrgencyLevel): number {
...
function calculateDelayPeriod(
  ...
  const urgencyFactor = (daysUntilDonation * URGENCY_FACTOR_WEIGHT / remainingBagsNeeded)
  return Math.round(Math.min(Math.max(minDelay, urgencyFactor), maxDelay))
}
