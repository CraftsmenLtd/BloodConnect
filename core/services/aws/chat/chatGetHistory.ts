...
async function chatGetHistoryLambda(
  event: GetChatHistoryEvent & HttpLoggerAttributes
  ...
  const messageService = new ChatMessageService(chatMessageDynamoDbOperations, httpLogger)

  try {
    const { requesterId, channelId, cursor, limit } = event
    const decodedCursor = cursor !== undefined && cursor !== '' ? decodeCursor(cursor) : undefined
    const page = await messageService.getHistory(
      channelId,
      requesterId,
      channelService,
      clampLimit(limit),
...
export default chatGetHistoryLambda
