...
async function pushNotificationMapper(
  event: NotificationAttributes
  ...
  try {
    ...
    const cachedUserSnsEndpointArn = userDeviceToSnsEndpointMap.get(
      event.userId
    );
    ...
    const errorCode =
      error instanceof NotificationOperationError
        ? error.errorCode
        : HTTP_CODES.ERROR;
