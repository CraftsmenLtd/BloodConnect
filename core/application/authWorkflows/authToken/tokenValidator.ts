...
export default function validateToken<T extends object>(token: string): T & { exp: number } {
  try {
    const decodedToken = verify(token, jwtSecret)
    return typeof decodedToken === 'string' ? JSON.parse(decodedToken) : decodedToken
  } catch (e) {
