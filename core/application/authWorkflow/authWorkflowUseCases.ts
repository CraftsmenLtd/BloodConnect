...
import {
  getDaysInSecs,
  getRemainingSecsOfDay
} from '../../../commons/libs/dateTimeUtils'
...
import { executionEnv } from '../../application/authWorkflow/authToken/constants'
...
export function getRefreshToken(
  ...
  return getJwtToken(
    tokenPayload,
    getDaysInSecs(RefreshTokenValidityInDaysForClient[clientType])
  )
}
...
export function getAuthToken(
  ...
  const expiresIn =
    expiresInSeconds > 0 ? expiresInSeconds : getRemainingSecsOfDay()
...
export function getAuthTokenFromRefreshToken(
  refreshToken?: string
): string | undefined {
  try {
    ...
    appLogger(executionEnv).info(
      'Refresh token validation error',
      (err as InvalidTokenError).message
    )
  }
}

export function getPayloadFromBearerToken(
  bearerToken: string
): object | undefined {
